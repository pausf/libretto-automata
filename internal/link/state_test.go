...
func TestScanOrderIsStable(t *testing.T) {
	...
	for i := range first {
		// reflect.DeepEqual rather than !=: Entry carries the generated bytes for a
		// transformed kind, and a struct holding a slice is not comparable. The
		// assertion is unchanged — two scans of one tree agree, byte for byte.
