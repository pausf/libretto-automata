...
What `libretto install` puts into a target, and nothing else. Mostly by symlink; an
OpenCode agent is a derived file this tool writes, and Codex takes skills only — so
which of these reach a given target depends on the target.

`scripts/` and `docs/` are not installed, so no skill, agent or command may reference
this page.
