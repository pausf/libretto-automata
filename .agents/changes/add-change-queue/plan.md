...
- [ ] 1. `commands/libretto-queue.md` — the capture loop; defines the `Queued:` line
      convention. Waits on: nothing.
      Closes: "both commands parse and every skill they reference exists" —
      scripts/check-payload
- [ ] 2. `commands/libretto-next.md` — pick oldest-first, branch, remove `Queued:`,
      enter phase 2. Waits on: 1 (the convention it reads).
      Closes: scripts/check-payload
- [ ] 3. `skills/find-work/SKILL.md` — queued proposals reported, never blocking, never
      source 1. Waits on: 1.
      Closes: scripts/check-payload
- [ ] 4. `commands/libretto-status.md` — the queue section, delegated to find-work's
      scan. Waits on: 3.
      Closes: scripts/check-payload
